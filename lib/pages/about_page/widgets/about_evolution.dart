...
class AboutEvolution extends StatelessWidget {
  ...
  List<Widget> getEvolution(Pokemon pokemon) {
    ...
    if (pokemon.prevEvolution != null) {
      pokemon.prevEvolution.forEach((f) {
        _list.add(resizePokemon(_pokeApiStore.getImage(numberPokemon: f.num)));
        _list.add(
          Padding(
    ...
    _list.add(resizePokemon(
        _pokeApiStore.getImage(numberPokemon: _pokeApiStore.pokemonActual.num)));
    _list.add(
      Padding(
        padding: const EdgeInsets.only(top: 8.0, bottom: 8.0),
        child: Text(
          _pokeApiStore.pokemonActual.name,
          style: TextStyle(
            fontSize: 16,
            fontWeight: FontWeight.bold,
          ),
    ...
    return Container(
      child: Padding(
        ...
        child: Observer(builder: (context) {
          Pokemon pokemon = _pokeApiStore.pokemonActual;
